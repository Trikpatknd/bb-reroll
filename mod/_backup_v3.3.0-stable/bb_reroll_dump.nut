...
::BBReroll_BF_Run <- function (worldState) {
    ...
    local doCleanup = function () {
        ...
        // Scenarios add items to the player's stash AND resize it (gladiators
        // shrinks by -9 every onSpawnAssets call). Without a reset, the stash
        // capacity drifts negative after ~10 iters and BB crashes inside
        // stash_container.resize. Empty items, then re-pin capacity.
        try {
            ...
            if (items != null) {
                foreach (it in items) {
                    if (it != null) toRemove.append(it);
                }
            }
            foreach (it in toRemove) {
                try { stash.remove(it); } catch (e) {}
            }
            try { stash.resize(200); } catch (e) {}
        } catch (e) {}
        // Reset accumulating resources/reputation. 20 origins mutate Money,
        // 14 Ammo, 12 Medicine, 9 ArmorParts, 16 Reputation — without resets
        // each runs cumulatively across the loop. None crashes today but
        // pinning to 0 each iter is defense-in-depth (~zero perf cost).
        try {
    ...
    for (local x = 0; x < ::BBReroll_BF.MaxIters; x++) {
        ...
        if (x < 3 || x % ::BBReroll_BF.LogEvery == 0) {
            ::logInfo(Tag + " iter " + x + " seed=" + seed + " — gen world");
        }
        ...
        try {
            ...
            // Reseed for deterministic bro generation. The hook on
            // starting_scenario also reseeds, but doing it here too means the
            // result is identical even if the hook didn't take on some scenario.
            ...
            if (pass) {
                ::logWarning(Tag + " *** MATCH at iter " + x + " seed=" + seed + " ***");
